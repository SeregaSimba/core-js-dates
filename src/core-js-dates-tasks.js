...
function getNextFriday(date) {
  ...
  const newWeek = week.getUTCDay();

  const resWeek = (5 - newWeek + 7) % 7;

  if (resWeek === 0) {
    week.setUTCDate(week.getUTCDate() + 7);
  } else {
    week.setUTCDate(week.getUTCDate() + resWeek);
  }
  return week;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const firstWeek = new Date(dateStart);
  const secondsWeek = new Date(dateEnd);
  const resWeek = secondsWeek - firstWeek;
  const day = resWeek / 1000 / 60 / 60 / 24;
  return day + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  const newPerStr = new Date(period.start).getTime();
  const newPerEnd = new Date(period.end).getTime();

  return newPerStr <= newDate && newDate <= newPerEnd;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  while (newDate.getMonth() === month - 1) {
    if ([6, 0].includes(newDate.getDay())) {
      obj += 1;
    }
...
function getWeekNumberByDate(date) {
  const newDate = new Date(date.getFullYear(), 0, 1, 0, 0, 0, 0);
  const getTim = newDate.getTime();
  const secGetTime = date.getTime();
  const getWeek = Math.ceil(
    ((secGetTime - getTim) / 1000 / 60 / 60 / 24 + 1) / 7
  );
  return getWeek;
}
